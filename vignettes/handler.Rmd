...
wq <- WebQueue$new(handler = function (req) { capture.output(dput(req[sort(names(req))])) })

httr2::request('http://localhost:8080/dir/file.txt?c=world') |>
  httr2::req_headers('Cookie' = 'token=abc', 'x-session-id' = 123) |>
  httr2::req_body_json(list(a=c(1,1,3),b='hello')) |>
  httr2::req_dry_run()
...
parse <- local({
  counter <- 1
  function (req) {
    req$ARGS$counter <- counter
    counter <<- counter + 1
  }
})

wq <- WebQueue$new(
  handler = function (req) { req$ARGS$counter },
  parse   = parse )
