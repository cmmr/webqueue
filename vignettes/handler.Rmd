...
To construct more complex HTTP response, use the `response()`, `header()`, 
...
The `parse` function is called on `req` (an environment). Aside from `req$ARGS`
and `req$COOKIES`, `req` is exactly as received from `httpuv`. After this
callback, extraneous `httpuv` fields are removed from `req` to minimize the 
amount of data sent to the background process.
...
environments. However, any changes made to `req` by `handler` will not be 
reflected here.
