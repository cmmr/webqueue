...
A request (`req`) is a `list` of data sent from a web browser to the web server. A [Job](jobqueue::Job) (`job`) is an R6 object containing a single `req` along with the parameters for processing it.
...
The handler does not have access to `job`. To interrupt a job from within its
...
Use the `stop_id` field to only run the most request with a given hash.
