...
wq <- webqueue(handler, stop_id = stop_id)

# Fetch three URLs at the same time.
jq <- jobqueue::jobqueue(workers = 3L)
...
Note: in reality, you'd want to use `webqueue::webqueue(onHeaders)` to do 
this particular task more efficiently.


### Limit Concurrency

Once the job is assigned to a `jobqueue`, you can access the list of all jobs
...
r3$result
#> [1] "interrupt: Too many concurrent jobs.\n"
